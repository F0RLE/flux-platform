...
pub fn get_stats() -> SystemStats {
    ...
    // Optimized refresh: only request CPU usage, not frequency/vendor info every time
    monitor
        .sys
        .refresh_cpu_specifics(CpuRefreshKind::nothing().with_cpu_usage());
    monitor.sys.refresh_memory(); // Memory is cheap
    monitor.networks.refresh(true);
    // Refresh processes for Disk I/O is handled below (throttled)
    ...
    // Disk Usage Calculation (Throttled to every 5s to save CPU)
    if monitor.last_disk_update.elapsed() >= Duration::from_secs(5) {
        ...
        let disk_elapsed = monitor.last_disk_update.elapsed().as_secs_f64();
        if disk_elapsed > 0.0 {
    ...
    // Disk Activity Calculation (Max 500 MB/s)
    ...
    // Network Activity Calculation (Max 100 MB/s)

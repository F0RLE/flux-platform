...
pub trait ModuleLifecycle {
    ...
    fn dispose(&mut self) -> Result<(), String> { self.stop() }
    fn health_check(&self) -> ModuleHealth { ModuleHealth::Unknown }
}
...
fn default_api_version() -> String { "1".to_string() }
...
pub fn load_manifest(module_dir: &PathBuf) -> Result<ModuleManifest, String> {
    let manifest_path = module_dir.join("module.json");
    if !manifest_path.exists() { return Err(format!("Manifest not found")); }

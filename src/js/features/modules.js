...
function renderModules(items) {
    ...
    if (!items || items.length === 0) {
        grid.innerHTML = `<div style="color: var(--text-muted); font-size: 0.85rem;">${t('ui.launcher.web.no_modules', 'Модули пока не добавлены.')}</div>`;
        return;
    }
    ...
    function statusHint(status, installed) {
        ...
        if (installed && status === 'installed') return t('ui.launcher.web.module_status.installed', 'Модуль установлен');
        if (status === 'running') return t('ui.launcher.web.module_status.running', 'Сервис запущен');
        if (!installed && status === 'available') return t('ui.launcher.web.module_status.available', 'Модуль можно скачать');
        return '';
    }
                                ...
                                ${installed ? `
                                    ...
                                    <button class="module-action-btn module-action-btn-primary"
                                        ...
                                        <button class="module-start-stop-btn ${isRunning ? 'running' : 'stopped'}"
                                                ...
                                                title="${isRunning ? t('ui.launcher.web.module_btn.stop', 'Остановить') : t('ui.launcher.web.module_btn.start', 'Запустить')}">
                                        </button>
                                    </div>
                                ` : canInstall ? `
        ...
        sections.push(`<div class="modules-category-section">
                <div class="modules-category-header">
                    <div class="modules-category-header-left">
                        ...
                        <div>
                            <h2 class="modules-category-title">${t('ui.launcher.modules.category.text_generation', 'Генерация текста')}</h2>
                        </div>
                    </div>
                    <button class="modules-category-add-btn" onclick="window.addModule('textGeneration')" title="${t('ui.launcher.modules.add_module_title', 'Добавить модуль')}">
                        <svg class="icon" style="width: 16px; height: 16px;"><use href="#icon-plus"></use></svg>
                        <span class="add-btn-text">${t('ui.launcher.modules.add_btn', 'Добавить')}</span>
                    </button>
                </div>
        ...
        sections.push(`<div class="modules-category-section">
                <div class="modules-category-header">
                    <div class="modules-category-header-left">
                        ...
                        <div>
                            <h2 class="modules-category-title">${t('ui.launcher.modules.category.image_generation', 'Генерация изображений')}</h2>
                        </div>
                    </div>
                    <button class="modules-category-add-btn" onclick="window.addModule('imageGeneration')" title="${t('ui.launcher.modules.add_module_title', 'Добавить модуль')}">
                        <svg class="icon" style="width: 16px; height: 16px;"><use href="#icon-plus"></use></svg>
                        <span class="add-btn-text">${t('ui.launcher.modules.add_btn', 'Добавить')}</span>
                    </button>
                </div>
        ...
        sections.push(`<div class="modules-category-section">
                <div class="modules-category-header">
                    <div class="modules-category-header-left">
                        ...
                        <div>
                            <h2 class="modules-category-title">${t('ui.launcher.modules.category.general', 'Общие сервисы')}</h2>
                        </div>
                    </div>
                    <button class="modules-category-add-btn" onclick="window.addModule('generalServices')" title="${t('ui.launcher.modules.add_module_title', 'Добавить модуль')}">
                        <svg class="icon" style="width: 16px; height: 16px;"><use href="#icon-plus"></use></svg>
                        <span class="add-btn-text">${t('ui.launcher.modules.add_btn', 'Добавить')}</span>
                    </button>
                </div>
    ...
    if (sections.length === 0) {
        grid.innerHTML = `<div style="color: var(--text-muted); padding: 4rem 2rem; text-align: center; grid-column: 1 / -1;"><div style="font-size: 1.1rem; margin-bottom: 0.5rem;">${t('ui.launcher.web.no_modules', 'Нет модулей')}</div><div style="font-size: 0.9rem; opacity: 0.7;">${t('ui.launcher.web.start_adding_module', 'Начните с добавления модуля')}</div></div>`;
    } else {
...
window.toggleModule = async function (moduleId) {
    ...
    try {
        ...
        showToast(t('ui.launcher.web.module_status_updated', 'Статус модуля обновлен'), 'success', 2000, t('ui.launcher.web.success', 'Успешно'));
    } catch (e) {
        showActionFeedback('error');
        showToast(t('ui.launcher.web.error_prefix', 'Ошибка: ') + e.message, 'error', 3000, t('ui.launcher.web.status_error', 'Ошибка'));
    } finally {
        if (toggleBtn) {
...
function showAddModuleModal(category) {
    showToast(t('ui.launcher.web.feature_future', 'Функция добавления модулей будет доступна в будущем'), 'info');
}
...
async function showModuleSettingsModal(moduleId) {
    ...
    try {
        ...
        if (module) {
            ...
            document.getElementById('module-settings-desc').textContent = module.description || t('ui.launcher.web.module_settings', 'Настройки модуля');

...
// Voice Input using Web Speech API (Native, optimized)
...
window.toggleVoiceInput = function () {
    ...
    if (!('webkitSpeechRecognition' in window)) {
        showToast(t('ui.launcher.web.voice_not_supported', 'Голосовой ввод не поддерживается'), 'error');
        return;
    }
    ...
    try {
        ...
        // Settings for "Understand All Languages" - try to use app language or system default
        // We map common short codes to full locales for better accuracy
        let lang = window.currentLang || navigator.language || 'en-US';
        ...
        recognition.onstart = function() {
            ...
            if (chatInput) {
                chatInput.setAttribute('data-placeholder-prev', chatInput.placeholder);
                chatInput.placeholder = t('ui.launcher.web.voice_listening', 'Слушаю...');
            }
        };

        recognition.onend = function() {
            // Auto-restart if we didn't explicitly stop (for continuous listening checks)
            // But usually we just stop UI updates here
            if (isRecording) {
                 // Verify if we should restart?
                 // For now, let's treat silence/end as stop to prevent "stuck" loops
                 stopVoiceRecording();
            }
        };
        ...
        recognition.onresult = function(event) {
             ...
             if (chatInput) {
                 // Improve: Don't overwrite existing text unless necessary
                 // We append final results. Interim results could be shown as placeholder or temp text.
                 // Simple approach: Input value = (Initial + Final + Interim)?
                 // Better: Append Final to Value.

                 if (finalTranscript) {
                     // Add space if needed
                     const val = chatInput.value;
                     const prefix = val && !val.endsWith(' ') ? ' ' : '';
                     chatInput.value = val + prefix + finalTranscript;
                 }

                 // Note: Interim results handling in a textarea is tricky without cursor jumping.
                 // We will skip showing interim in the input value directly to avoid UX mess,
                 // or we could show it in a floating tooltip.
                 // For now, simple "Append Final" is robust.
             }
        };
        ...
        recognition.onerror = function(event) {
            ...
            if (event.error === 'not-allowed') {
                 showToast(t('ui.launcher.web.voice_denied', 'Доступ к микрофону запрещен'), 'error');
            }
            stopVoiceRecording();
        };
...
window.stopVoiceRecording = function () {
    ...
    if (chatInput) {
        const prev = chatInput.getAttribute('data-placeholder-prev');
        if (prev) chatInput.placeholder = prev;
        chatInput.focus();
    }
};
...
// Keyboard shortcuts moved to window.js
...
// Chat Greeting Randomization
function randomizeChatGreeting() {
    const keys = [
        'ui.launcher.web.chat_question', // Default
        'ui.launcher.web.chat_question_1',
        'ui.launcher.web.chat_question_2',
        'ui.launcher.web.chat_question_3',
        'ui.launcher.web.chat_question_4',
        'ui.launcher.web.chat_question_5'
    ];
    // Randomly select one
    ...
    if (el) {
        // Update the data-i18n attribute so language switching works seamlessly
        el.setAttribute('data-i18n', randomKey);

        // Update the text immediately if t() is available
        if (typeof t === 'function') {
            const defaultText = {
                'ui.launcher.web.chat_question': 'What are you focusing on?',
                'ui.launcher.web.chat_question_1': 'How can I help you today?',
                'ui.launcher.web.chat_question_2': "Let's create something new",
                'ui.launcher.web.chat_question_3': 'Any ideas in mind?',
                'ui.launcher.web.chat_question_4': 'Time to be productive',
                'ui.launcher.web.chat_question_5': 'System ready. Awaiting instructions'
            }[randomKey];
            el.textContent = t(randomKey, defaultText);
        }
    }
}

// Randomize on initial load
document.addEventListener('DOMContentLoaded', randomizeChatGreeting);

// Also randomize when tab becomes visible if it's the chat tab
document.addEventListener('visibilitychange', () => {
    if (!document.hidden) {
        // Optional: only randomize if enough time passed?
        // For now, let's just keep it simple. Usually users want stability during session.
        // But the user asked for "variety".
        // Let's randomize ONLY on load for now to avoid it changing while they are looking at another tab and coming back.
    }
});

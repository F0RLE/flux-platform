let translations = {};
// let currentLang = 'en'; // Removed local variable to use shared global
window.currentLang = window.currentLang || 'en'; // Expose to window for other modules
...
async function getSystemLanguage() {
    try {
        ...
        console.warn("Failed to get system language from backend:", e);
    }

    // Fallback: Check browser navigator
    const navLang = navigator.language || navigator.userLanguage || 'en';
    if (navLang.toLowerCase().includes('ru')) {
        return 'ru';
    }
    if (navLang.toLowerCase().includes('zh') || navLang.toLowerCase().includes('cn')) {
        return 'zh';
    }
    return 'en';
}
...
async function loadTranslations(lang = null) {
    try {
        ...
        document.documentElement.lang = langToLoad; // For CSS targeting

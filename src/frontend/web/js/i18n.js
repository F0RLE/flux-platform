...
async function loadTranslations(lang = null) {
    try {
        ...
        const res = await fetch(`/api/translations?lang=${langToLoad}&t=${Date.now()}`);
        if (!res.ok) {
        ...
        if (lang) {
            ...
            // Update LANGUAGE (launcher language) in backend - bot will use this if BOT_LANGUAGE is not set
            try {
                ...
                // Also update BOT_LANGUAGE to empty/remove it so bot uses LANGUAGE
                // This ensures bot always follows launcher language
                await fetch('/api/settings', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        applyTranslations();
        initEmojiFlags();
        updateLangButtons(); // Updates the UI with the current lang
        if (typeof updateCurrentLangFlag === 'function') {
            updateCurrentLangFlag(); // Update the language button flag
        }
    } catch (e) {
